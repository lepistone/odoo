        ...
        def get_real_price_curency(res_dict, product_id, qty, uom, pricelist):
            ...
            if rule_id:
                ...
                if item_base > 0:
                    ...
                    product_read = product_obj.read(cr, uid,
                                                    [product_id], [field_name],
                                                    context=context)[0]
                    base_price = product_read[field_name]
                else:
                    assert item_base == -1
                    # based on other pricelist
                    base_price_list_id = item_obj.read(cr, uid,
                                                       [rule_id],
                                                       ['base_pricelist_id'])[0]['base_pricelist_id'][0]
                    pl_obj = self.pool.get('product.pricelist')
                    base_pl = pl_obj.browse(cr, uid, [base_price_list_id])[0]

                    rule_match = base_pl.price_get(product_id, qty)
                    assert len(rule_match) == 1
                    base_price = rule_match.values()[0]
            ...
            return base_price * factor, currency_id
        ...
        if product and pricelist and self.pool.get('res.users').has_group(cr, uid, 'sale.group_discount_per_so_line'):
            ...
            if so_pricelist.visible_discount: # and list_price[pricelist][0] != 0 and new_list_price != 0:
                ...
                discount = (new_list_price - price) / new_list_price * 100 \
                    if new_list_price else 100.0 if price else 0.0
                if discount > 0:
